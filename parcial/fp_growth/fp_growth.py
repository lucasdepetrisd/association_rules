...
with open('rules_fpgrowth.pkl', 'wb') as f:
    pickle.dump(rules, f)

# Calcular métricas adicionales
rules['leverage'] = rules.apply(lambda x: 
    x['support'] - (x['antecedent support'] * x['consequent support']), axis=1)
rules['conviction'] = rules.apply(lambda x: 
    (1 - x['consequent support']) / (1 - x['confidence']) if x['confidence'] < 1 else np.inf, axis=1)
